...
| `SphereEmbeddable.of_maxDegree_le` (FKS Proposition 2) | partial. Proved: base `d = 2` (`of_degree_le_two`), the step `d ≥ 4` (`of_degree_le_ge_four`), the reduction of base `d = 3` to two-regular graphs (`of_degree_le_three_of_regular`), Proposition 2 conditional on it (`of_degree_le_of_three`, `of_degree_le_of_regular`), every cycle and every **connected** two-regular graph on `𝕊²` (`Sphere/Cycles.lean`: `exists_cycle_placement`, `cycleGraph`, `of_connected_degree_two`). **Open: disconnected two-regular graphs** (components placed with non-colliding offsets) |
