...
| `unitDistEmbeddable_of_card_edges_eq` (rung 4) | **`SimpleGraph.nonempty_iso_completeGraph_of_not_unitDistEmbeddable`** (`d ≥ 6`, `C(d + 2, 2)` edges, no isolated vertex, no placement in `ℝᵈ` → `G ≃g K_{d+2}`), `Extremal/FKS/Uniqueness.lean`, with the equality-budget branches in `EqualityBranches.lean`. Statement as the owner chose it; its Stage 1 freeze and Compass are still to do |
